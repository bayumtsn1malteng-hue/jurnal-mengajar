...
import { ChevronLeft, Plus, Calendar, Filter, Save, Search } from 'lucide-react';
import { db } from '../db';
...
const NilaiPage = () => {
  ...
  const [allAssessments, setAllAssessments] = useState([]); // Ganti nama biar jelas ini MASTER
  ...
  const loadAssessmentList = async () => {
    try {
      ...
      const joined = meta.map(m => ({
        ...
        // Kita butuh nama topik untuk filter
        topicName: '?' // Nanti diisi kalau perlu, tapi filter pakai ID saja cukup
      }));
  ...
  useEffect(() => {
    const initData = async () => {
        ...
        if (cls.length > 0) {
            ...
            // Default Filter: Pilih kelas pertama biar tidak kosong melompong
  ...
  // --- FITUR BARU: LOGIKA FILTER (DERIVED STATE) ---
  const filteredAssessments = useMemo(() => {
    return allAssessments.filter(item => {
      // 1. Filter Kelas (Jika ada filter, harus cocok. Jika kosong, lolos semua)
  ...
  // --- FITUR BARU: PESAN DINAMIS ---
  const getEmptyMessage = () => {
    if (allAssessments.length === 0) return "Belum ada data nilai sama sekali.";
    
    // Cari Nama Kelas & Topik untuk pesan error
    const clsName = classes.find(c => c.id === parseInt(filterClassId))?.name || 'Semua Kelas';
    const topicName = syllabusList.find(t => t.id === parseInt(filterTopicId))?.topic || 'Semua Topik';
    
    return `Tidak ada latihan untuk topik "${topicName}" di kelas ${clsName}`;
  };

  // --- LOGIKA LAIN (SAMA SEPERTI SEBELUMNYA) ---
  const filteredTemplates = selectedTopicId
    ...
    <div className="min-h-screen bg-slate-50 pb-24">
      ...
      <div className="bg-white p-6 rounded-b-3xl shadow-sm sticky top-0 z-20 mb-6">
        <div className="flex items-center justify-between">
            ...
            {view === 'list' && <button onClick={() => setView('create')} className="p-3 bg-indigo-600 text-white rounded-2xl shadow-lg"><Plus size={24} /></button>}
        </div>

        {/* --- FITUR BARU: FILTER SECTION (Hanya muncul di List View) --- */}
        {view === 'list' && (
            <div className="mt-6 grid grid-cols-2 gap-3 animate-in slide-in-from-top-2">
                ...
                <div className="relative">
                    ...
                    <select 
                        ...
                        className="w-full pl-9 p-2.5 bg-indigo-50 border-none rounded-xl text-xs font-bold text-indigo-900 focus:ring-2 focus:ring-indigo-300 outline-none"
                    >
                ...
                <div className="relative">
                    ...
                    <select 
                        ...
                        className="w-full pl-9 p-2.5 bg-teal-50 border-none rounded-xl text-xs font-bold text-teal-900 focus:ring-2 focus:ring-teal-300 outline-none"
                    >
      ...
      <div className="p-6 pt-0">
           ...
           <div className="space-y-4">
             ...
             {filteredAssessments.length === 0 ? (
                 ...
                 filteredAssessments.map(item => (
                    <div key={item.id} onClick={() => openInputPage(item)} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition cursor-pointer group">
        ...
        {view === 'create' && (
           /* --- BAGIAN FORM CREATE (Tidak Berubah) --- */
        ...
        {view === 'input' && !loading && (
             ...
             <div className="space-y-4">
                ...
                <div className="fixed bottom-24 left-6 right-6 max-w-md mx-auto"><button onClick={handleSaveGrades} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-xl flex justify-center gap-2"><Save/> Simpan</button></div>
             </div>
        )}
      </div>
    </div>
